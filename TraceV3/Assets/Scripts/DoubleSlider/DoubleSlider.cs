...
namespace TS.DoubleSlider
{
    ...
    public class DoubleSlider : MonoBehaviour
    {
        ...
        [SerializeField] private float _velocityFactor = 0.1f;
        [SerializeField] private float _resetVelocityThreshold = 0.5f;
        public float MinIncrement { get; private set; }
        public float MaxIncrement { get; private set; }
        public float LowerValue { get; private set; }
        public float HigherValue { get; private set;}
        ...
        private void MinValueChanged(float value)
        {
            ...
            {
                _sliderMax.Value = Mathf.Min(_sliderMax.Value + _minDistance, _maxValue);
            }
            if (Mathf.Abs(_sliderMin.Velocity) < _velocityThreshold)
            {
                MinIncrement += (_sliderMin.Velocity > 0 ? _minDistance : -_minDistance) * _velocityFactor;
            }
            else
            {
            ...
            Debug.Log("Min Increment" + MinIncrement);
        ...
        private void MaxValueChanged(float value)
        {
            ...
            {
                _sliderMin.Value = Mathf.Max(_sliderMin.Value - _minDistance, _minValue);                 
            }
            if (Mathf.Abs(_sliderMax.Velocity) < _velocityThreshold)
            {
               MaxIncrement += (_sliderMax.Velocity > 0 ? _minDistance : -_minDistance) * _velocityFactor;
            }
            else
            {
            ...
            Debug.Log("Max Increment"+ MaxIncrement);
            OnValueChanged.Invoke(MinValue, MaxValue);
        }
    }
}